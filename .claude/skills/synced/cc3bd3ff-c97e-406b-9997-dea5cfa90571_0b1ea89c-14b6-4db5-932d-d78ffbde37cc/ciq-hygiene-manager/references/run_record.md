...
- **Layer B — the rep skill (operational).** Reps fix their own deals (Next Step, MEDDPICC, labels, line items,
  `blockers`) and stamp the deal-level `hygiene_score`/`last_hygiene_score_date` on what they fix — so a fix shows
  immediately instead of waiting for the next batch. The rep skill never writes the object.
...
Clean deal → 100; `weighted_open=10` → 50; `=30` → 25. Written by Layer A (every deal, 2×/day) and Layer B (deals the rep
fixes). `last_hygiene_score_date` = the date it was last computed.
...
gaps). **`blocker uncaptured` is ADVISORY, NOT scored** — the batch never penalizes an empty `blockers` field (it only reads it
as a hold-until date that *suppresses* flags). Surface it as a nudge; it must not lower a deal's score. **Close<->stage coherence (D-40) IS scored** (CRITICAL early-stage-imminent, WARNING overdue) AND joins the **forecast-threat set** (dark / stalled / stale-next-step / thin-MEDDPICC + close-coherence) -> it raises forecast-at-risk.
