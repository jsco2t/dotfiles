...
- **Do NOT write the `Hygiene Run` object** (`2-63704277`). The manager skill is **read-only** — it reads the records the
  **twice-daily server-side batch** (`scripts/score_all_reps.py`) writes; it never creates records. (The connector can't
  create custom-object records anyway.) On-demand scoring/verification is a fresh in-chat snapshot, not a write.
