...
39. **A bare month WORD in `blockers` is NOT a hold date (parse_hold "may" bug; D-42, 2026-06-09).** `parse_hold` read the
    English word "may" ("partner **may** have existing RHEL/SUSE contracts") as month May -> a false 2027-05-01 hold that
    suppressed dark / stalled / stale-next-step on 11 cold deals (Ani Fox scored 88 on never-contacted deals). Fix: a month
    NAME only counts as a hold-until date with a day/year ("May 2027", "Sept 15") or a hold cue just before it ("paused until
    March"); bare "may"/"march" are ordinary words. Golden-tested (`tests/test_rules.py`; `lib/ciq_rules.py` `parse_hold`).
